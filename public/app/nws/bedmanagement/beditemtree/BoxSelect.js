 ...
 *
 */
Ext.define('com.dfsoft.icu.nws.bedmanagement.beditemtree.BoxSelect', {
    ...
    /**
     * @cfg {Boolean}
     * 当emptyText有值并且此参数设置为ture时，点击文本框时隐藏默认文本并使输入框获焦
     */
    focusOnClick: true,
    ...
    initEvents: function() {
        ...
        me.mon(me.listWrapper, 'click', function(evt, el, o){
            var eve = evt,ell =el,oo =o;
            if(me.editData){
                me.onItemListDblclick(evt, el, o);
                /*if(!clickTimer){
                    ...
                    clickTimer = null;
                    me.onItemListDblclick(eve, ell, oo);
                }*/
            }else{
                me.onItemListClick(eve, ell, oo);
            }
    ...
    createPicker: function() {
        ...
        picker.addCls('x-boxselect-picker');
    ...
    alignPicker: function() {
        ...
        picker.loadMask.el&&picker.loadMask.el.setVisibilityMode(2);
        picker.loadMask.maskEl&&picker.loadMask.maskEl.setVisibilityMode(2);
        me.callParent(arguments);
    ...
    onKeyUp: function(e, t) {
        ...
        if(this.keyup&&!this.keyup(e, t,me.inputOldValue)){
            me.inputOldValue = rawValue;
            return false
        }
        me.callParent([e,t]);
    },
    ...
    onItemListClick: function(evt, el, o) {
        ...
        if (itemEl) {
            ...
            if(this.emptyEl.isVisible()&&me.focusOnClick){
                me.onFocus({});
                me.applyEmptyText();
                me.inputEl.focus();
            }
    ...
    onItemListDblclick:function(evt, el, o){
        var me=this,rec,inputDom,
            itemEl = evt.getTarget('.x-boxselect-item'),
            itemWidth,
            closeEl = itemEl ? evt.getTarget('.x-boxselect-item-close') : false;
        //判断是否只读
        ...
        if (me.selectionModel.getCount() > 0) {
            me.selectionModel.setLastFocused(null);
            me.selectionModel.deselectAll(true);
            var selDom = evt.currentTarget.getElementsByClassName('selected');
            for(var i =0;i<selDom.length;i++){
                selDom[i].className = "x-boxselect-item";
            }
        }
        if (itemEl) {
            //如果点的删除按钮
            ...
            if(!me.editSelectData){
                if(rec.get(me.codeName)!=''){
                    return false;
                }
            }
            if(!rec){
            ...
            itemEl.childNodes[0].style.display='none';
            itemEl.childNodes[1].style.visibility = 'hidden';
            itemEl.childNodes[2].style.display='';
            me.inputElCt.setStyle('display','none');
            inputDom = itemEl.childNodes[2];
            var DomText = rec.get('text');
            //通过鼠标点击的位置计算出光标移动到的位置
            inputDom.innerText = '';
            for(var wz = 1;inputDom.offsetWidth<evt.browserEvent.layerX;wz++){
                inputDom.innerText = DomText.slice(0,wz)
            }
            //可编辑DIV赋值
            inputDom.innerText = rec.get('text');
            inputDom.focus();
            //移动光标
            var sel = window.getSelection();
            sel.modify('move','left','documentboundary');
            for(var width = 1;width<wz;width++){
                sel.modify('move','right','character');
            }
            inputDom.onclick=function(e){
                e.stopPropagation();
            }
            inputDom.onkeyup = function(e){
                var iValue = this.innerText.split('；'),
                    eValue = this.innerText.split(';'),
                    endValue;
                if(iValue.length>1||eValue.length>1){
                    endValue = iValue.length>1?iValue:eValue;
                    var oldText = inputDom.parentElement.getAttribute('data-qtip').trim();
                    for(var oi=0;oi<me.value.length;oi++){
                        if(me.value[oi] == oldText){
                            if(endValue[1].length>0){
                                inputDom.onblur = null;
                                this.innerText = '';
                                me.value[oi] = endValue[1];
                                if(endValue[0].length>0){
                                    me.value.splice(oi,0,endValue[0]);
                                }

                            }else if(endValue[0].length>0){
                                inputDom.onblur = null;
                                this.innerText = '';
                                me.value[oi] = endValue[0];
                            }
                            me.setValue(me.value);
                            me.inputElCt.setStyle('display','');
                            break;
                        }
                    }
                }
            ...
            inputDom.onkeypress=function(e){
                if(e.which == 13){
                    ...
                    e.stopPropagation();
                    e.preventDefault();
                }/*else if(String.fromCharCode(e.which) == me.delimiter){
                    var starto = window.getSelection().getRangeAt().startOffset,
                        iValue = this.innerText;
                    var firstS = iValue.slice(0,starto),
                        endS=iValue.slice(starto,iValue.length);
                    var oldText = inputDom.parentElement.getAttribute('data-qtip').trim();
                    for(var oi=0;oi<me.value.length;oi++){
                        if(me.value[oi] == oldText){
                            me.value[oi] = endS;
                            me.value.splice(oi,0,firstS);
                            me.setValue(me.value);
                            break;
                        }
                    }
            ...
            inputDom.onselectstart=function(e){
                e.stopPropagation();
            }
            inputDom.onblur=function(){
                var oldText = inputDom.parentElement.getAttribute('data-qtip').trim(),
                    newText = inputDom.innerText.trim();
                if(oldText != newText){
                    if(inputDom.innerText.trim()==''){
                        me.removeByListItemNode(itemEl);
                        if (me.valueStore.getCount() > 0) {
                            me.fireEvent('select', me, me.valueStore.getRange());
                        }
                    }else{
                        rec.set('text',inputDom.innerText);
                        rec.set('value','');
                        itemEl.childNodes[0].style.display='';
                        itemEl.childNodes[1].style.visibility='';
                        itemEl.childNodes[2].style.display='none';
                        me.setValue(me.valueStore.getRange());
                    }
                }else{
                    itemEl.childNodes[0].style.display='';
                ...
                me.inputElCt.setStyle('display','')
                inputDom.onblur = null;
                inputDom.onkeypress = null;
                inputDom.onclick = null;
                var scDom = me.el.dom.getElementsByClassName('x-boxselect');
                if(scDom.length>0){
                    scDom = scDom[0];
                    if(scDom.scrollHeight>scDom.offsetHeight&&scDom.scrollTop>0){
                        me.inputEl.dom.focus();
                    }else if(scDom.scrollHeight==scDom.offsetHeight&&scDom.scrollTop==0){
                        me.inputEl.dom.focus();
                    }
                }
                //me.inputEl.dom.focus();
    ...
    getMultiSelectItemMarkup: function() {
        ...
        if (!me.multiSelectItemTpl) {
            ...
            me.multiSelectItemTpl = [
                ...
                /*TODO：增加多项分隔符*/
                    '<div class="x-boxselect-item-text">{[typeof values === "string" ? values : this.getItemLabel(values)]}',
                '{[xindex !=xcount ? "'+me.delimiter+'" : ""]}',
                '</div>',
                ...
                '<div contenteditable="true" class="x-boxselect-item-text reEditDiv" style="color:darkgreen;background:#f5f5f5;white-space:nowrap;height:100%;margin:0;border:0;display:none;"></div>' ,
                '</li>' ,
                '</tpl>',
                {
                    compile: true,
    ...
    getJsonValue: function() {
        var valueArray = this.valueStore.getRange();
        valueObj=[];
        for(var i = 0 ;i<valueArray.length;i++){
            valueObj.push(valueArray[i].raw)
        }
        return valueObj;
    },
...
Ext.define('Ext.ux.layout.component.field.BoxSelectField', {
    ...
    publishInnerWidth:function(ownerContext) {
        var me = this,
            owner = me.owner,
            width = owner.itemList.getWidth(true),
...
muiltInput={
    ...
    setValue:function(id,value){
        var Input = Ext.getCmp(id+'_MuiltInput');
        if(value&&Input){
            var valueArray=value.split(',');
            Input.addValue(valueArray);
        }
    },
    //传入input的ID绑定插件
    bindInput:function(config, renderTo) {
    	//封装选择器
        ...
        var me=this,
            oInputDom = $("#"+renderTo)[0],
            parentDom = oInputDom.parentElement,
            iWidth = oInputDom.offsetWidth;
        //隐藏原有input
        oInputDom.style.visibility='hidden';
        parentDom.style.verticalAlign = 'middle';
        parentDom.style.position='relative';
        parentDom.style.overflowX='hidden';
        oInputDom.style.minHeight='20px';
        //获取input提示信息
        var placeholder = oInputDom.placeholder;
        //页面缩放时缩放触发父级对象函数
        me.resizeMuiltDom.push(parentDom);
        //插件默认配置项
        var fieldCfg = Ext.applyIf(config, {
            displayField: 'text',//显示字段
            valueField: 'text',//value字段
            storeUrl:'',
            multiSelect:true,//是否为多选
            codeName:'value',//model中code字段名
            storeLocalArray:null,//本地数据数组
            id:renderTo+'_MuiltInput',//插件ID
            value:oInputDom.value,
            width: iWidth,
            notDeep:false,//
            editData:false,//是否可编辑已输入数据
            editSelectData:false,//是否可编辑下拉选框选择的数据
            growMax:22,//插件最大高度
            growMin:22,
        ...
        if(!fieldCfg.store){
            fieldCfg.store=muiltInput.getStore(fieldCfg.pageSize||0,fieldCfg.queryMode,fieldCfg.storeUrl,fieldCfg.storeLocalArray,fieldCfg.codeName,fieldCfg.notDeep);
        }
        //插件数据更改后自动同步到input中
        fieldCfg.listeners.change=function(){
            //var valueArray = this.getJsonValue()
            oInputDom.value = this.getValue();
            if(config.changeFn){
                config.changeFn(this);
            }
        }
        ...
        //缩放事件触发后自动适应宽度
        parentDom.onresize=function(){
            fieldCmp.setWidth(this.offsetWidth);
        }
        //如果内容换行，则阻止滚动事件冒泡
        parentDom.onmousewheel=function(e){
            var scrDom = this.getElementsByClassName('x-boxselect'),scrollH;
            if(scrDom.length>0){
                scrDom = scrDom[0];
                scrollH = scrDom.offsetHeight;
                if(scrDom.scrollHeight>scrollH){
                    if(e.wheelDelta>0){
                        scrollH = scrollH *-1;
                    }
                    scrDom.scrollTop += scrollH;
                    e.stopPropagation();
                    e.preventDefault();
                }
            }
    ...
    getStore:function(limit, queryMode, url, localArray,codeName,notDeep) {
        var store = null;
        var root = null;
        ...
        if (queryMode) {
            if (queryMode === 'remote'&&!localArray) {
                store = Ext.create('Ext.data.Store', {
                    pageSize: limit,
                    fields: ['text', codeName],
                    autoLoad: true,
                    proxy: {
                        type: 'ajax',
                        url: url,
                    ...
                    listeners: {
                        load: function(_this, records, successful, eOpts) {
                            ...
                            if (size <= 0) {
                                if (url.indexOf('searchUser') == -1) {
                                    _this.loadData([], true);
                                } else {
                                    _this.loadData([], true);
                                }
                            }else if(_this.getAt(0).raw.surgery_code == ''){
                                _this.clearData()
                            }
                        }
                    }
