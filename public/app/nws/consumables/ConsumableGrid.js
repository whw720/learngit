 ...
 * @author: zag
 */
Ext.define('com.dfsoft.icu.nws.consumables.ConsumableGrid', {
    ...
    initComponent: function () {
        ...
        me.rowEditing = Ext.create('Ext.grid.plugin.RowEditing', {
            ...
            listeners: {
                beforeedit: function (editor, context, eOpts) {
                    ...
                    var delObj = Ext.ComponentQuery.query('button[name=delCon]', this.ownerCt)[0];
                    var fastObj = Ext.ComponentQuery.query('button[name=fastCon]', this.ownerCt)[0];
                    fastObj.disable();
                    delObj.disable();
                    //console.log(items[0]);
                ...
                validateedit: function (editor, context, eOpts) {
                    ...
                    if (conRecode.data.CONSUMABLES_CODE == "") {
                ...
                canceledit: function (editor, context) {
                    ...
                    Ext.each(items, function (item) {
                        if (item.data.ID == "") {
                            ...
                            var fastObj = Ext.select('a.x-row-editor-cancel-button');
                            fastObj.elements[0].click();
                ...
                edit: function (editor, context) {
                    ...
                    var orderTypeObj = Ext.ComponentQuery.query('combo[name=doctor-order-type-bc]', context.grid.ownerCt.ownerCt)[0];
                    var orderTypeValue = orderTypeObj.getValue();

                    var conRecode = context.grid.getSelectionModel().lastSelected;
                    //console.log(conRecode.data);
                    var useConParams = {
                        ID: conRecode.data.ID,//耗材使用唯一标识
                        SCHEDULING_TYPE: orderTypeValue,//使用班次
                        USE_DATE: conRecode.data.USE_DATE,//使用日期
                        CONSUMABLES_CODE: conRecode.data.CONSUMABLES_CODE,//使用耗材代码
                        AMOUNT: conRecode.data.AMOUNT,//用量
                        REGISTER_ID: me.registerId,//患者登记ID
                    ...
                    if (conRecode.data.ID == "") {//保存
                        Ext.Ajax.request({
                            ...
                            success: function (response) {
                                ...
                                conRecode.set('ID', reqmsg.data.conId);
                        ...
                        var totalPrice = 0;
                        var amount = useConParams.AMOUNT;
                        var itemCode = useConParams.CONSUMABLES_CODE;
                        Ext.Ajax.request({
                            url: parent.webRoot + '/nws/updateConsumable/' + useConParams.ID,
                            method: 'PUT',
                            params: {
        ...
        var currentTime = new Date();// 传入护理时间
        var startTime = "";
        var endTime = "";
        var orderType = "";
        //currentTime = currentTime.Format("yyyy-MM-dd");
        if (currentTime.getHours() < 7 || currentTime.getHours() > 19) {
            //夜班起始时间
            //  startTime = currentTime.getFullYear()+"-"+(currentTime.getMonth() + 1) + "-"+(currentTime.getDate()) + " 19:00:00";
            //  endTime = currentTime.getFullYear()+"-"+(currentTime.getMonth() + 1) + "-"+(currentTime.getDate() + 1) + " 7:00:00";
            orderType = "c7605a40abf011e396e800271396a820";
        } else {
            //白班起始时间
            // startTime = currentTime.getFullYear()+"-"+(currentTime.getMonth() + 1) + "-"+(currentTime.getDate()) + " 7:00:00";
            //  endTime = currentTime.getFullYear()+"-"+(currentTime.getMonth() + 1) + "-"+(currentTime.getDate()) + " 19:00:00";
            orderType = "be0df177abf011e396e800271396a820";
        }
        startTime = currentTime.Format("yyyy-MM-dd 00:00:00");
        me.store = Ext.create('com.dfsoft.icu.nws.consumables.ConsumableStore', {
                       ...
                       matchFieldWidth: false,
                        ...
                        listeners: {
                            ...
                            select: function (combo, records, eOpts) {
                                ...
                                for (var i = 0; i < medicineBeforeAnaesthesiaStore.getCount(); i++) {
                                    var record = medicineBeforeAnaesthesiaStore.getAt(i);
                                    if (record.data.CONSUMABLES_CODE == records[0].data.CODE) {// 选择项目已经添加过。
                                ...
                                if (!flag) {
                                   ...
                                   // console.log(records[0]);
                                    ...
                                    recordCurrent.set('CONSUMABLES_CODE', records[0].data.CODE);//使用耗材代码
                                    ...
                                    var elitems = Ext.fly(me.mod + "consumablepanel-body").select("div.x-form-display-field[role=input]");
                                    var amountitems = Ext.fly(me.mod + "consumablepanel-body").select("input.x-form-field[name=AMOUNT]");
                                    ...
                                    elitems.elements[5].innerHTML = totalPrice.toFixed(2);
                                    elitems.elements[6].innerHTML = records[0].data.RECORD_CODE;
                                }
                    ...
                    editor: {
                        ...
                        listeners: {
                            change: function (_this, newValue, oldValue, eOpts) {
                                var elitems = Ext.fly(me.mod + "consumablepanel-body").select("div.x-form-display-field[role=input]");
                                var price = elitems.elements[4].innerHTML;
                                if (price != "") {
                                    elitems.elements[5].innerHTML = (newValue * price).toFixed(2);
                                }
                            }
                        }
                    }
