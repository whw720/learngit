 ...
 * @version 2013-3-21
 */
Ext.define('com.dfsoft.icu.nws.nursingscores.vpossum.VPossumForm', {
    extend: 'Ext.panel.Panel',
    alias: 'widget.vpossumform',
    requires: ['com.dfsoft.icu.nws.nursingscores.vpossum.ValueCount',
        'com.dfsoft.icu.nws.nursingscores.vpossum.VPossumStoreGrid',
    ...
    /*分值项目录入验证*/
    itemVerification: function (possumitem) {
        var me = this;

        if (possumitem.vpossum.Eyes == "") {
            Ext.Msg.alert("提示", "请选择睁眼反应");
            me.itemscore = "";
            return false;
        }
        if (possumitem.vpossum.Language == "") {
            Ext.Msg.alert("提示", "请选择语言反应");
            me.itemscore = "";
            return false;
        }
        if (possumitem.vpossum.Motor == "") {
    ...
    initComponent: function () {
        ...
        me.vpossumstoregrid = new com.dfsoft.icu.nws.nursingscores.vpossum.VPossumStoreGrid();
        me.changCalc = new com.dfsoft.icu.nws.nursingscores.vpossum.ValueCount();
        ...
        Ext.apply(this, {
            title: 'V-POSSUM',
            closable: true,
            tooltip: '手术死亡和并发症生理学和手术参数评分（血管）',
            id:me.mod + 'vpossumform',
            layout: 'fit',
            autoScroll: true,
            dockedItems: [
                {
                    ...
                    items: [
                        ...
                        ,
                        {
                            ...
                            handler: function (btn) {
                                ...
                                var toppanel = Ext.getCmp(btn.ownerCt.ownerCt.mod + 'vpossumtoppanel');
                                var intervalTime = 24;
                                //创建遮罩效果
                                var lm = new Ext.LoadMask(this.ownerCt,{
                                    msg: "数据加载中。。。"
                                });
                                lm.show();
...

                        {
                            ...
                            handler: function (btn) {
                                ...
                                var toppanel = Ext.getCmp(me.mod + 'vpossumtoppanel');
                                //表单验证.
                                ...
                                //拼装项目明细字串
                                me.itemscore = '{"vpossum":{' + me.itemscore.substr(0, me.itemscore.length - 1) + '}}';
                                //转换为对象
                                var vpossumItem = Ext.JSON.decode(me.itemscore);
                                veri = me.itemVerification(vpossumItem);
                                if (!veri) {
                                    return;
                                }
                                // 获取系数规则
                                Ext.Ajax.request({
                                    url: parent.webRoot + '/app/nws/nursingscores/vpossum/ChangeScores.json',
                                    async: false,
                                ...
                                //计算总得分
                                scores = me.changCalc.scoresCount(ChangeScores, vpossumItem);
                                ...
                                //显示计算结果窗口
                                var cw = Ext.create('com.dfsoft.icu.nws.nursingscores.vpossum.CountWindow',
                                    {
                                        morta: morta,
        ...
        var scoreCode = "641ca522b18711e3aa8800271396a820";
    ...
    load: function (data, patientId) {
        ...
        if (Ext.isArray(data) && data.length > 0) { //如果不是空则填充页面数据
         ...
         /*   var possumitem = data[data.length-1].CONTENT;
            ...
            me.vpossumstoregrid.reconfigure(tablestore);

        }else{
            me.operType = 'add';
            me.chartstoreT.loadData("");
            me.vpossumstoregrid.getStore().loadData("");
        }
        me.elm.hide();
    },
// 控件赋值
    setItemValue:function(itemobj,posssumdataobj){
        var me = this;
        if (itemobj.name === undefined) {
            if (itemobj.items === undefined) {
                return '';
            ...
            Ext.each(itemobj.items.items, function (item) {
                return me.setItemValue(item,posssumdataobj);
            })
        } else {
            if (itemobj.xtype == "radiofield") {
            } else {
                for (var p in posssumdataobj.vpossum){
                    if(itemobj.name == p){
                        if(eval("posssumdataobj.vpossum."+p) != "null" && eval("posssumdataobj.vpossum."+p) != ""){
                            itemobj.setValue(eval("posssumdataobj.vpossum."+p));
                        }
                    }
                }
            }
