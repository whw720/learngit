 ...
 * @version 2013-3-21
 */
Ext.define('com.dfsoft.icu.nws.nursingscores.opossum.OPossumForm', {
    extend: 'Ext.panel.Panel',
    alias: 'widget.opossumform',
    requires: ['com.dfsoft.icu.nws.nursingscores.opossum.ValueCount',
        'com.dfsoft.icu.nws.nursingscores.opossum.OPossumStoreGrid',
    ...
    /*分值项目录入验证*/
    itemVerification: function (opossumitem) {
        var me = this;
        if (opossumitem.opossum.Eyes == "") {
            Ext.Msg.alert("提示", "请选择睁眼反应");
            me.itemscore = "";
            return false;
        }
        if (opossumitem.opossum.Language == "") {
            Ext.Msg.alert("提示", "请选择语言反应");
            me.itemscore = "";
            return false;
        }
        if (opossumitem.opossum.Motor == "") {
    ...
    initComponent: function () {
        ...
        me.changCalc = new com.dfsoft.icu.nws.nursingscores.opossum.ValueCount();
        me.opossumstoregrid = new com.dfsoft.icu.nws.nursingscores.opossum.OPossumStoreGrid();
        ...
        Ext.apply(this, {
            title: 'O-POSSUM',
            closable: true,
            tooltip: '手术死亡和并发症生理学和手术参数评分（上消化道）',
            id: me.mod + 'opossumform',
            layout: 'fit',
            autoScroll: true,
            dockedItems: [
                {
                    ...
                    items: [
                        ...
                        ,
                        {
                            ...
                            handler: function (btn) {
                                ...
                                var toppanel = Ext.getCmp(btn.ownerCt.ownerCt.mod + 'opossumtoppanel');
                                var intervalTime = 24;
                                //创建遮罩效果
...

                        {
                            ...
                            handler: function (btn) {
                                ...
                                var toppanel = Ext.getCmp(me.mod + 'opossumtoppanel');
                                //表单验证.
                                ...
                                //拼装项目明细字串
                                me.itemscore = '{"opossum":{' + me.itemscore.substr(0, me.itemscore.length - 1) + '}}';
                                var opossumItem = Ext.JSON.decode(me.itemscore);
                                veri = me.itemVerification(opossumItem);
                                if (!veri) {
                                    return;
                                }
                                // 获取系数规则
                                Ext.Ajax.request({
                                    url: parent.webRoot + '/app/nws/nursingscores/opossum/ChangeScores.json',
                                    async: false,
                                ...
                                //计算总得分
                                scores = me.changCalc.scoresCount(ChangeScores,opossumItem);
                                ...
                                //显示计算结果窗口
                                var cw = Ext.create('com.dfsoft.icu.nws.nursingscores.opossum.CountWindow',
                                    {
                                        morta: morta,
                        ...
                        '-',
                        {
                            ...
                            iconCls: 'ico-help',
                            handler: function (btn) {
                                var hw = Ext.create('com.dfsoft.icu.nws.nursingscores.HelpWindow',{itempath:'opossum'});
                                var pmod = btn.ownerCt.ownerCt.mod;
        ...
        var scoreCode = "640c8983b18711e3aa8800271396a820";
...
/*            var scoreItemStr = "";
            ...
            me.opossumstoregrid.reconfigure(tablestore);
        }else{
            me.operType = 'add';
            me.chartstoreT.loadData("");
            me.opossumstoregrid.getStore().loadData("");
        }
        me.elm.hide();
    },
// 控件赋值
    setItemValue:function(itemobj,oposssumdataobj){
        var me = this;
        if (itemobj.name === undefined) {
            if (itemobj.items === undefined) {
                return '';
            ...
            Ext.each(itemobj.items.items, function (item) {
                return me.setItemValue(item,oposssumdataobj);
            })
        } else {
            if (itemobj.xtype == "radiofield") {
            } else {
                for (var p in oposssumdataobj.opossum){
                    if(itemobj.name == p){
                        if(eval("oposssumdataobj.opossum."+p) != "null" && eval("oposssumdataobj.opossum."+p) != ""){
                            itemobj.setValue(eval("oposssumdataobj.opossum."+p));
                        }
                    }
                }
            }
