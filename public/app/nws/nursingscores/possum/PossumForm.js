 ...
 * @version 2013-3-21
 */
Ext.define('com.dfsoft.icu.nws.nursingscores.possum.PossumForm', {
    extend: 'Ext.panel.Panel',
    alias: 'widget.possumform',
    requires: ['com.dfsoft.icu.nws.nursingscores.possum.ValueCount',
        'com.dfsoft.icu.nws.nursingscores.possum.PossumStoreGrid',
    ...
    itemVerification: function (possumitem) {
        ...
        if (possumitem.possum.Eyes == "") {
            Ext.Msg.alert("提示", "请选择睁眼反应");
            me.itemscore = "";
            return false;
        }
        if (possumitem.possum.Language == "") {
            Ext.Msg.alert("提示", "请选择语言反应");
            me.itemscore = "";
            return false;
        }
        if (possumitem.possum.Motor == "") {
    ...
    initComponent: function () {
        ...
        me.changCalc = new com.dfsoft.icu.nws.nursingscores.possum.ValueCount();
        me.possumstoregrid = new com.dfsoft.icu.nws.nursingscores.possum.PossumStoreGrid();
        ...
        Ext.apply(this, {
            title: 'POSSUM',
            closable: true,
            tooltip: '手术死亡和并发症生理学和手术参数评分',
            id: me.mod + 'possumform',
            layout: 'fit',
            autoScroll: true,
            dockedItems: [
                {
                    ...
                    items: [
                        ...
                        ,
                        {
                            ...
                            handler: function (btn) {
                                ...
                                var toppanel = Ext.getCmp(btn.ownerCt.ownerCt.mod + 'possumtoppanel');
                                var intervalTime = 24;
                                //创建遮罩效果
...

                        {
                            ...
                            handler: function (btn) {
                                ...
                                var toppanel = Ext.getCmp(me.mod + 'possumtoppanel');
                                //表单验证.
                                ...
                                //拼装项目明细字串
                                me.itemscore = '{"possum":{' + me.itemscore.substr(0, me.itemscore.length - 1) + '}}';
                                //转换为对象
                                var possumItem = Ext.JSON.decode(me.itemscore);
                                veri = me.itemVerification(possumItem);
                                if (!veri) {
                                    return;
                                }
                                // 获取系数规则
                                Ext.Ajax.request({
                                    url: parent.webRoot + '/app/nws/nursingscores/possum/ChangeScores.json',
                                    async: false,
                                ...
                                //计算总得分
                                scores = me.changCalc.scoresCount(ChangeScores, possumItem);
                                ...
                                //显示计算结果窗口
                                var cw = Ext.create('com.dfsoft.icu.nws.nursingscores.possum.CountWindow',
                                    {
                                        morta: morta,
        ...
        var scoreCode = "63fef784b18711e3aa8800271396a820";
    ...
    load: function (data, patientId) {
     ...
     /*       var possumitem = data[data.length-1].CONTENT;
            ...
            me.possumstoregrid.reconfigure(tablestore);


        }else{
            me.operType = 'add';
            me.chartstoreT.loadData("");
            me.possumstoregrid.getStore().loadData("");
        }
        me.elm.hide();
    },
// 控件赋值
    setItemValue:function(itemobj,posssumdataobj){
        ...
        if (itemobj.name === undefined) {
            ...
            if (itemobj.xtype == "radiofield") {
                ...
                for (var p in posssumdataobj.possum){
                    if(itemobj.name == p){
                        if(eval("posssumdataobj.possum."+p) != "null" && eval("posssumdataobj.possum."+p) != ""){
                            itemobj.setValue(eval("posssumdataobj.possum."+p));
                        }
                    }
                }
            }
