 ...
 * @version 2013-3-21
 */
Ext.define('com.dfsoft.icu.nws.nursingscores.apachetwo.ApacheTwoForm', {
                    ...
                    items: [
                        ...
                        '-',
                        {
                            ...
                            handler: function (btn) {
                                ...
                                //拼装项目明细字串
                                me.itemscore = '{"apache2":{' + me.itemscore.substr(0, me.itemscore.length - 1) + '}}';
                                //转换为对象
                                var apache2item = Ext.JSON.decode(me.itemscore);
                                //各种验证
                                veri = me.itemVerification(apache2item);
                                if (!veri) {
                                    return;
                                }
                                // 获取系数规则
                                Ext.Ajax.request({
                                    url: parent.webRoot + '/app/nws/nursingscores/apachetwo/ChangeScores.json',
                                    async: false,
                                ...
                                if (apache2item.apache2.pio < 0.5) {

                                    apache2item.apache2.aa = "null";
                                }

                                //计算总得分
                                scores = me.changCalc.scoresCount(ChangeScores, apache2item);
                                //计算死亡率
                                mr = me.changCalc.calcDeath(scores);
                                //项目明细json串
                                contentStr = me.itemscore;
                                //显示计算结果窗口
                                var cw = Ext.create('com.dfsoft.icu.nws.nursingscores.apachetwo.CountWindow',
                                    {
                                        scores: scores,
