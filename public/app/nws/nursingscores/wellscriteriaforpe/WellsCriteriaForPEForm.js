 ...
 * @version 2013-3-21
 */
Ext.define('com.dfsoft.icu.nws.nursingscores.wellscriteriaforpe.WellsCriteriaForPEForm', {
    extend: 'Ext.panel.Panel',
    alias: 'widget.wellscriteriaforpeform',
    requires: [
        'com.dfsoft.icu.nws.nursingscores.wellscriteriaforpe.WellsCriteriaForPEGrid',
    'com.dfsoft.icu.nws.nursingscores.wellscriteriaforpe.WellsCriteriaForPEItemGrid',
    ...
    initComponent: function () {
        ...
        me.wellscriteriaforpeGrid = new com.dfsoft.icu.nws.nursingscores.wellscriteriaforpe.WellsCriteriaForPEGrid();
        me.wellscriteriaforpeItmeGrid = new com.dfsoft.icu.nws.nursingscores.wellscriteriaforpe.WellsCriteriaForPEItemGrid();
        me.wellscriteriaforpeItmeGrid.addListener('itemclick', itemclick);
        ...
        Ext.apply(this, {
            title: 'Well’s Criteria for PE',
            closable: true,
            tooltip: '肺栓塞风险预测',
            id: me.mod + 'wellscriteriaforpeform',
            layout: 'fit',
            autoScroll: true,
            dockedItems: [
                {
                    ...
                    items: [
...

                        {
                            ...
                            handler: function (btn) {
                                ...
                                var rowCon = me.wellscriteriaforpeItmeGrid.getStore().getCount();
                                for (var j = 0; j < rowCon; j++) {
                                    var scoreValue = me.wellscriteriaforpeItmeGrid.getStore().getAt(j).get('SCORES');
                                    var sel = me.wellscriteriaforpeItmeGrid.getStore().getAt(j).get('SEL');
                                    if (sel) {
                                        scores = scores + scoreValue;
                                        contentStr = contentStr + '"wellscriteriaforpe_item' + j + '":"' + scoreValue + '",';
                                    }else{
                                        contentStr = contentStr + '"wellscriteriaforpe_item' + j + '":"",';
                                    }
                                }
                                contentStr = "{" + contentStr.substr(0, contentStr.length - 1) + "}";
                                contentStr = '{"wellscriteriaforpe":' + contentStr + '}';
                                if(scores == 0){
                                    Ext.Msg.alert("提示", "请选择项目!");
                                    return;
                                }

                                var cw = Ext.create('com.dfsoft.icu.nws.nursingscores.wellscriteriaforpe.CountWindow',
                                    {
                                        scores: scores,
                        ...
                        '-',
                        {
                            ...
                            iconCls: 'ico-help',
                            handler: function (btn) {
                                var hw = Ext.create('com.dfsoft.icu.nws.nursingscores.HelpWindow',{itempath:'wellscriteriaforpe'});
                                var pmod = btn.ownerCt.ownerCt.mod;
        ...
        var scoreCode = "9ab0ec1db18811e3aa8800271396a820";
...
//        toppanel.getForm().reset();
    ...
    load: function (data, patientId) {
        ...
        if (Ext.isArray(data) && data.length > 0) { //如果不是空则填充页面数据
            ...
            me.wellscriteriaforpeGrid.reconfigure(tablestore);
        }else{
            // 清空数据
            me.chartstoreT.loadData("");
            me.wellscriteriaforpeGrid.getStore().loadData("");
        }
        me.elm.hide();
    }
});