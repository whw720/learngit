  ...
  fabric.Canvas = fabric.util.createClass(fabric.StaticCanvas, /** @lends fabric.Canvas.prototype */ {
    ...
    containsPoint: function (e, target) {
      var pointer = this.getPointer(e),
          xy = {x: e.layerX, y: e.layerY};//this._normalizePointer(target, pointer);//王小伟 2014-04-03修改，div容器非absolute布局方式下鼠标定位错误
...
(function() {
  ...
  fabric.util.object.extend(fabric.IText.prototype, /** @lends fabric.IText.prototype */ {
    ...
    initHiddenTextarea: function() {
      ...
      this.hiddenTextarea.style.cssText = 'position: absolute; top: 0; left: -999px';
      var textObj = this;
      if (window.attachEvent){
        this.hiddenTextarea.attachEvent("onpropertychange", function(e){
          var newValue = e.srcElement.value;
          textObj.setChars(newValue);
        });
      } else if (window.addEventListener){
        this.hiddenTextarea.addEventListener("input", function(e){
          var newValue = e.target.value;
          newValue = newValue.slice(textObj.selectionStart, newValue.length);
          newValue = newValue.slice(0, newValue.length - textObj.text.slice(textObj.selectionStart, textObj.text.length).length);
          var reg = /^[\u4e00-\u9fa5]+$/;
          if (reg.test(newValue) && newValue !== textObj.text) {
            // textObj.setChars(newValue);
            textObj.insertChars(newValue);
          }
        });
      }
    ...
    /**
     * replace text from IText An Object.
     * @param {new chars}
     */
    setChars: function (_chars){
      var offsetCursor = _chars.length - this.text.length;
      ...
      this.selectionStart = this.selectionEnd = (this.selectionStart + offsetCursor);
      this.moveTextareaCursor();
    },

    /**
     * move the cursor of hidden textarea of IText.
     * @return {Number}
     */
    moveTextareaCursor: function(){
      if (document.selection) {
          var sel = this.hiddenTextarea.createTextRange();
          sel.moveStart('character',this.selectionStart);
          sel.collapse();
          sel.select();
      } else if (typeof this.hiddenTextarea.selectionStart == 'number' 
          && typeof this.hiddenTextarea.selectionEnd == 'number') {
    ...
    _removeCharsNearCursor: function(e) {
      if (this.selectionStart !== 0) {
        ...
        this.hiddenTextarea.value = this.text.trim();
        if (document.selection) {
            var sel = this.hiddenTextarea.createTextRange();
            sel.moveStart('character',this.selectionStart);
            sel.collapse();
    ...
    insertNewline: function() {
      this.hiddenTextarea.value = this.text.slice(0, this.selectionStart) +
                    '\n' +
                  this.text.slice(this.selectionEnd);
      this.insertChars('\n');
    }
